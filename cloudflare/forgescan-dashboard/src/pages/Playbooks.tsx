    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Automated response actions — isolate hosts, escalate incidents, enrich IOCs, create tickets</p>
        </div>
      ...
      {overview && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Playbooks', value: `${overview.totals.enabled}/${overview.totals.playbooks}`, sub: 'enabled', icon: Workflow, color: 'text-violet-400' },
            { label: 'Executions', value: overview.totals.total_executions, sub: 'total runs', icon: Zap, color: 'text-teal-400' },
            { label: 'Success Rate', value: `${overview.totals.success_rate}%`, sub: `${overview.totals.successful} passed`, icon: CheckCircle, color: 'text-emerald-400' },
            { label: 'Failed', value: overview.totals.failed, sub: 'runs failed', icon: XCircle, color: 'text-red-400' },
          ].map(s => (
      ...
      <div className="space-y-3">
        {playbooks.map(pb => {
          ...
          return (
            <div key={pb.id} className="p-4 rounded-lg bg-white/[0.03] border border-navy-400/15">
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-3">
                  <Workflow className="h-5 w-5 text-purple-400" />
                  <div>
                    <p className="text-sm font-medium text-white">{pb.name}</p>
                    <p className="text-xs text-muted-foreground">{pb.description || 'No description'}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                {steps.map((s, i) => (
                  <div key={i} className="flex items-center">
                    <span className="text-[10px] px-2 py-0.5 rounded bg-white/[0.06] text-muted-foreground">{s.action_type.replace(/_/g, ' ')}</span>
                    {i < steps.length - 1 && <span className="text-muted-foreground/40 mx-1">&rarr;</span>}
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => togglePlaybook(pb)} className="text-xs border-border text-muted-foreground hover:text-white">
        ...
        {playbooks.length === 0 && <p className="text-sm text-muted-foreground text-center py-12">No playbooks configured. Click "From Template" to install pre-built response workflows.</p>}
      </div>

      {/* Templates Dialog */}
      <Dialog open={showTemplates} onOpenChange={setShowTemplates}>
        <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5 text-teal-400" /> Playbook Templates</DialogTitle><DialogDescription>Install pre-built automated response playbooks.</DialogDescription></DialogHeader>
          <div className="space-y-3 mt-2">
            {templates.map((tmpl, i) => (
              <div key={i} className="p-4 rounded-lg bg-white/[0.04] border border-navy-400/15">
                <div className="flex items-start justify-between">
                  <div><p className="text-sm font-medium text-white">{tmpl.name}</p><p className="text-xs text-muted-foreground mt-1">{tmpl.description}</p></div>
                ...
                <div className="flex items-center gap-1 mt-2 flex-wrap">
                  {(tmpl.steps as Array<{ action_type: string }>).map((s, j) => (
                    <span key={j} className="text-[10px] px-1.5 py-0.5 rounded bg-white/[0.06] text-muted-foreground">{s.action_type.replace(/_/g, ' ')}{j < tmpl.steps.length - 1 ? ' \u2192' : ''}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      <Dialog open={!!execResult} onOpenChange={() => setExecResult(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Playbook Execution Result</DialogTitle></DialogHeader>
          {execResult && (
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">{execResult.steps_completed} steps completed</p>
              <div className="space-y-1">
                {execResult.step_results.map((sr, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-muted-foreground">{sr.result}</span>
                  </div>
                ))}
              </div>
            </div>
