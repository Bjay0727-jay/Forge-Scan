...
async fn main() -> Result<()> {
    ...
    if args.register {
        return run_registration(&platform_endpoint, &config_path).await;
    }
...
/// Run agent registration via REST
async fn run_registration(platform_endpoint: &str, config_path: &str) -> Result<()> {
    ...
    let payload = RegisterRequest {
        hostname: sys_info.os.hostname.clone(),
        version: env!("CARGO_PKG_VERSION").to_string(),
        os_name: sys_info.os.name.clone(),
        os_version: sys_info.os.version.clone(),
        os_arch: sys_info.os.arch.clone(),
        capabilities: vec!["config_audit".to_string()],
    };

    let client = reqwest::Client::new();
    let url = format!("{}/api/v1/agent/register", platform_endpoint);
    ...
    // Save agent_id and api_key to config file
    let config_content = format!(
        r#"# ForgeScan Agent Configuration (auto-generated by registration)
...
agent_id = "{}"
"#,
        platform_endpoint, reg_resp.api_key, reg_resp.agent_id
    );
...
async fn run_daemon(
    ...
    let mut last_scan_time: Option<String> = None;

    // Task polling interval (shorter than heartbeat)
    ...
    loop {
        tokio::select! {
            _ = heartbeat_timer.tick() => {
                ...
                let sys = SystemCollector::collect();
                let payload = HeartbeatRequest {
                    ...
                    last_scan_time: last_scan_time.clone(),
                };

                let url = format!("{}/api/v1/agent/heartbeat", platform_endpoint);
                match client
                    .post(&url)
                    ...
                    .await
                {
                    ...
                    Ok(resp) => {
                        warn!("Heartbeat returned status {}", resp.status());
                    }
                    Err(e) => {
            ...
            _ = poll_timer.tick() => {
                ...
                let url = format!(
                    "{}/api/v1/agent/tasks/next?agent_id={}",
                    platform_endpoint, agent_id
                );
                match client
                    .get(&url)
                    .header("X-Agent-Key", &api_key)
                    ...
                    .await
                {
                    ...
                    Ok(resp) if resp.status().is_success() => {
                        match resp.json::<TaskResponse>().await {
                            Ok(task_resp) => {
                                ...
                                // Run config audit
                                info!("Running config audit for task {}...", task.id);
                                let mut auditor = ConfigAuditor::new();
                                let audit_result = auditor.run_audit();

                                // Convert audit results to findings
                                let findings: Vec<serde_json::Value> = audit_result
                                    .results
                                    .iter()
                                    .filter(|r| !r.passed)
                                    .map(|r| {
                                        serde_json::json!({
                                            ...
                                            "vendor": "forgescan-agent",
                                            "vendor_id": r.check_id,
                                            "evidence": format!("Expected: {}, Actual: {}", r.expected, r.actual),
                                            "solution": r.remediation,
                                        })
                                    })
                                    .collect();
                                ...
                                let summary = format!(
                                    ...
                                    audit_result.summary.total_checks,
                                    audit_result.summary.passed,
                                    audit_result.summary.failed,
                                    audit_result.summary.errors,
                                );
                                ...
                                let result_url = format!(
                                    "{}/api/v1/agent/tasks/{}/results",
                                    platform_endpoint, task.id
                                );
                                match client
                                    .post(&result_url)
                                    .header("X-Agent-Key", &api_key)
                                ...
                                {
                                    Ok(resp) if resp.status().is_success() => {
                                        info!("Task {} results submitted successfully", task.id);
                                        last_scan_time = Some(chrono::Utc::now().to_rfc3339());
                                    }
                                    Ok(resp) => {
                                        error!(
                                            "Failed to submit results for task {}: status {}",
                                            task.id,
                                            resp.status()
                                        );
                                    }
                    ...
                    Ok(resp) => {
                        warn!("Task poll returned status {}", resp.status());
                    }
                    Err(e) => {
